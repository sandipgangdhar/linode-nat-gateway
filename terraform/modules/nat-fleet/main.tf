...
#   this same three-interface layout for those) -- see
...
#   controller/natctl/cloud_init.py's elastic-node path -- they render
#   the same three-interface + FRR layout by two different mechanisms
#   (a Terraform template here vs. Python there), so a change to one
#   without the other silently produces two different node shapes.
#
# -----------------------------------------------------
# Author:
# - Sandip Gangdhar
...
locals {
  # natctl-on-node's package content is not read/embedded here at all --
  # terraform/modules/artifacts uploads controller/natctl/*.py
  ...
  # Per-node instance_type override -- lets a floor node that's been
  # vertically-scaled by natctl's operator-driven
  ...
  # The first length(var.reserved_ip_pool) node_ids (by creation order) are covered
...
# variables.tf's reserved_ip_enabled docs and
# docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html §4.6 for the full rationale. Referencing this from linode_instance.node below (and
...
# Every floor node's reserved address, regardless of whether it came
...
# vlan_reserved_cidr must actually be nested inside vlan_cidr, or "select
# addresses from the reserved block, but configure the interface with
# vlan_cidr's own (wider) prefix length" (see node_vlan_ips/ipam_address
# above) produces an address the interface's own connected route
# wouldn't even cover -- the same class of bug as a VPC-sibling-subnet
# routing gap: a route/prefix that's too narrow silently makes part of
# an address space unreachable even though nothing else is wrong. Plain
# integer-range containment, same technique
# terraform/environments/example/main.tf already uses for its own
# same-VLAN-mode nesting check.
locals {
...
# separate physical Akamai hosts, closing the gap where
# a correlated physical-host failure taking out both buddy
# pair members leaves nothing to fail over to, since buddy conntrack sync
# and BGP IP failover alone only protect against a single node dying.
# Akamai caps a group at 5
...
resource "linode_instance" "node" {
  ...
  # from VPC instead. See docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html §1.3.
  interface {
    purpose = "public"
  }
  ...
  # packet through a peer instance to a non-VPC-member destination --
  # confirmed via a live two-Linode test with tcpdump on both ends, not
  # an assumption (see docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html §1.2).
  interface {
    purpose   = "vpc"
  ...
  metadata {
    # gzip before base64 -- cloud-init auto-detects and decompresses
  ...
  # NOTE on Network Helper (a known limitation, NOT fixed here): Linode's
  # Network Helper regenerates /etc/systemd/network/05-eth0.network on
  # every boot from this instance's CURRENT IP-sharing state, and it
  # renders every ipv4.shared address (buddy IP failover's whole
  # mechanism -- ip_failover_enabled, POST /networking/ips/share) as an
  # extra static `Address=` line on eth0 -- not something FRR/BGP
  # announces dynamically from lo the way it's designed to work (see
  # docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html §4.4). Both buddies can end up
  # with the SAME address bound to eth0 simultaneously, breaking that
  # node's own egress outright, and it recurs on every reboot.
  # linode/linode provider ~> 2.9 (pinned in versions.tf) exposes
...
# Per-node dynamic config, uploaded to Object Storage instead of
...
locals {
  ...
  # but wiring them into the live NAT rules is a manual day-2 step
  # (scripts/add-egress-ip.sh) for the same underlying reason.
  ...
  node_cloud_init = {
    for node_id in local.node_ids : node_id => templatefile("${path.module}/../../../ansible/cloud-init/nat-node.yaml.tftpl", {
      ...
      # Routes eth1 to every sibling VPC subnet -- see
      # nat-node.yaml.tftpl's own runcmd comment for the full write-up.
      ...
      # nftables.conf is fetched at boot from the per-node Object Storage
      # object uploaded above, not embedded as content: here -- see this
      # file's top-of-file comment and
      # ansible/cloud-init/nat-node.yaml.tftpl's own header note for why
      # that alone wasn't enough headroom.
      nftables_conf_url = "${local.object_storage_base_url}/${linode_object_storage_object.nftables_conf[node_id].key}"

      # exporter.py/buddy_sync.py are fetched at boot from Object
      ...
      # systemd TEMPLATE unit buddy_sync.py's apply_conntrack_peers()
      # instantiates per conntrack peer (conntrackd@<peer_node_id>.service)
      # -- see buddy.py's compute_conntrack_peers() and
      # docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html §4.2 ("Buddy Pairing: the
      # 2-Node Pair and the 3-Node Triangle").
      conntrackd_peer_service_url = var.conntrackd_peer_service_url

      # natctl-on-node (opt-in) -- see variables.tf's
      # natctl_on_node_enabled and docs/NAT-GATEWAY-DEFINITIVE-GUIDE.html
      # §2.3/§2.4. Mirrors
      ...
      # controller/natctl/cloud_init.py's matching comment. The three
      # *_bin_url vars are cheap to pass even when disabled, same
      # reasoning as natctl_file_urls above.
