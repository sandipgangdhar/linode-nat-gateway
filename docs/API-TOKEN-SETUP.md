...
A token with `*` (unscoped, full account access) will work, but it is not least-privilege: it can also read/modify every other resource on your account — billing, other projects' instances, Kubernetes clusters, everything — regardless of whether LNG ever touches them. If this token ever leaks (a compromised CI runner, an accidentally-committed file, a misconfigured log), the blast radius should be limited to what LNG actually needs, not your whole account. An unscoped token is easy to end up with by default — most Linode CLI/API tokens are created that way unless a scope is deliberately chosen — so use a dedicated, scoped token for LNG specifically, not whatever broader token you may already have lying around.
...
If a raw token value ever ends up somewhere it shouldn't have (a command's output, a pasted log, a session transcript) — rotate it immediately, don't wait. Treat this the same way for any other credential this project uses (an Object Storage key, `root_pass`, `grafana_admin_password`): fast, complete rotation beats trying to prevent every possible exposure vector in advance.
