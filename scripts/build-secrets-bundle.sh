...
# key (the one that can decrypt it -- see .sops.yaml), then re-encrypts
# the whole plaintext as one age-encrypted blob for the recipient public
# key that matches whichever age PRIVATE key your nodes have at
# /etc/natctl/age-key.txt (the one Terraform writes from
# secrets_bundle_age_private_key in that same secrets.enc.json). This is
# a genuinely different encryption step from the SOPS one above it --
...
# --decrypt` streams straight into `age --encrypt` through a pipe.
#
# -----------------------------------------------------
# Usage:
#
...
sops --decrypt "$SECRETS_FILE" | age --encrypt --recipient "$RECIPIENT" --output "$OUT_FILE"
